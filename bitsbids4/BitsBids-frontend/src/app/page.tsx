...
import { cookies } from "next/headers"; // For Next.js to fetch cookies
import axios from "axios";

async function getProducts() {
  try {
    const userId = cookies().get("userId")?.value || "defaultUserId"; // Use userId from cookies
    const response = await axios.get(
      `http://localhost:8081/api/v1/products/user?userId=${userId}` // Update URL with dynamic userId
    );
...
export default async function Home() {
  ...
  const fallbackProduct = {
    productId: 0,
    // image: "sample-image-url", // Commented out the image
            ...
            <div className="container px-4 md:px-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
                {displayedProducts.map(
                  (product: {
                    productId: number;
                    // image: string; // Commented out image
                    productName: string;
                    details: string;
                  }) => (
                    <div
